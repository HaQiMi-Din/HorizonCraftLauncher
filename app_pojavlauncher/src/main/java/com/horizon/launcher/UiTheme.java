...
public final class UiTheme {
    ...
    /** Resolve the theme's primary text color so views stay legible in light and dark mode. */
    public static int getTextPrimaryColor(Context ctx) {
        ...
        if (ctx.getTheme().resolveAttribute(android.R.attr.textColorPrimary, tv, true)) {
            ...
            if (tv.resourceId != 0) {
                try { return ctx.getColor(tv.resourceId); } catch (Exception ignored) {}
            }
        }
        return ctx.getResources().getColor(R.color.ui_text_primary);
    }
